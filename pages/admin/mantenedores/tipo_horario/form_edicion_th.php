<?php
    ...
    $consulta ="SELECT * FROM tipo_horario WHERE id_th='$id_recibido'";
...
<title>Editar tipo de horario - KindomJob's</title>
    ...
    <table class="table table-striped table-bordered">
        ...
        <?php
            ...
            while($row = mysqli_fetch_assoc($resultado))
            {
                ...
                $nombre_th = $row["horario"];
                $id_nombre_th = $row["id_th"];
                if ($id == $id_nombre_th) {
        ?>
        <tr>
            <td>
                <form action='index.php?p=admin/mantenedores/tipo_horario/editar_th' method='POST'>
                    <div class="input-group mb-3">
                        <input type="hidden" name="id_e" value="<?php echo $id_nombre_th ?>">
                        <input type="text" name="nombre_e" class="form-control" aria-describedby="button-addon2"
                            value="<?php echo $nombre_th ?>" style="width:50%" required>
                        <button class="btn btn-outline-success" type="sumbit" id="button-addon">Guardar</button>
                    </div>
                </form>
            </td>
            <td>
                <a href="index.php?p=admin/mantenedores/tipo_horario/borrar_th&id_e=<?php echo $id_nombre_th ?>" class="btn btn-danger"
                    style="color: white; text-decoration: none;"><span class="material-icons">delete</span></a>
                <a href="index.php?p=admin/mantenedores/tipo_horario/form_edicion_th&id_e=<?php echo $id_nombre_th ?>" class='btn btn-primary'
