<?php
    ...
    $consulta ="SELECT *FROM estado_usuario WHERE id_estado_usuario='$id_recibido'";
...
<title>Editar estado de usuario - KindomJob's</title>
    ...
    <table class="table table-striped table-bordered">
        ...
        <?php
            ...
            while($row = mysqli_fetch_assoc($resultado))
            {
                ...
                $nombre_estado_usuario = $row["nombre_estado_usuario"];
                $id_nombre_estado_usuario = $row["id_estado_usuario"];
                if ($id == $id_nombre_estado_usuario) {
        ?>
        <tr>
            <td>
                <form action='index.php?p=admin/mantenedores/estado_usuario/editar_estado_usuario' method='POST'>
                    <div class="input-group mb-3">
                        <input type="hidden" name="id_e" value="<?php echo $id_nombre_estado_usuario ?>">
                        <input type="text" name="nombre_e" class="form-control" aria-describedby="button-addon2"
                            value="<?php echo $nombre_estado_usuario ?>" style="width:50%" required>
                        <button class="btn btn-outline-success" type="sumbit" id="button-addon">Guardar</button>
                    </div>
                </form>
            </td>
            <td>
                <a href="index.php?p=admin/mantenedores/estado_usuario/borrar_estado_usuario&id_e=<?php echo $id_nombre_estado_usuario ?>" class="btn btn-danger"
                    style="color: white; text-decoration: none;"><span class="material-icons">delete</span></a>
                <a href="index.php?p=admin/mantenedores/estado_usuario/form_edicion_estado_usuario&id_e=<?php echo $id_nombre_estado_usuario ?>" class='btn btn-primary'
        ...
        <?php
                ...
                echo "<td>".$nombre_estado_usuario."</td>";
                echo "<td>";
