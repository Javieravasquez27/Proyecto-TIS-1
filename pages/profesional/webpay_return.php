<?php
    ...
    try {
        ...
        if ($result->isApproved()) {
            ...
            if ($estrellas >0) {
                $quitarestrellas = "update usuario set estrellas = estrellas - $estrellas where rut = '$_SESSION[rut]'";
                $result = mysqli_query($conexion, $quitarestrellas);
            ...
            if (mysqli_query($conexion, $sql)) {
                ...
                header("Location: index.php?p=profesional/webpay_final&status=success&buyOrder=$buyOrder&amount=$amount&token=$tokenWs&rut_prof=$rut_prof&nombre_profesional=$nombre_profesional&nombre_servicio=$nombre_servicio&fecha_cita=$fecha_cita&hora_cita=$hora_cita&lugar_atencion=$lugar_atencion&token=$tokenWs&estrellas_gastadas=$estrellas");
            } else {
                echo "Error al guardar la cita: " . mysqli_error($conexion);
            }
        } else {
            header("Location: index.php?p=profesional/webpay_final&status=error&buyOrder=$buyOrder&amount=$amount&token=$tokenWs&rut_prof=$rut_prof&nombre_profesional=$nombre_profesional&nombre_servicio=$nombre_servicio&fecha_cita=$fecha_cita&hora_cita=$hora_cita&lugar_atencion=$lugar_atencion&token=$tokenWs&estrellas_gastadas=$estrellas");
        }
    } catch (Exception $e) {
